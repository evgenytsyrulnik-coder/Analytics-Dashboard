              ...
              <Line
                ...
                activeDot={user?.role === 'ORG_ADMIN' ? (props: any) => (
                  <circle cx={props.cx} cy={props.cy} r={6} fill="#10b981" stroke="white" strokeWidth={2}
                    ...
                    onClick={() => {
                      const date = props.payload.timestamp.split('T')[0];
                      navigate(`/org/runs?from=${date}&to=${date}&team_id=${teamId}&status=SUCCEEDED`);
                    }}
                  />
                ) : undefined}
              />
              <Line
                ...
                activeDot={user?.role === 'ORG_ADMIN' ? (props: any) => (
                  <circle cx={props.cx} cy={props.cy} r={6} fill="#ef4444" stroke="white" strokeWidth={2}
                    ...
                    onClick={() => {
                      const date = props.payload.timestamp.split('T')[0];
                      navigate(`/org/runs?from=${date}&to=${date}&team_id=${teamId}&status=FAILED`);
                    }}
                  />
                ) : undefined}
              />
